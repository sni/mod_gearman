...
my $load = ""; # Will be filled later by randomness
my $temp_hostname;
my $temp_servicename;
my $temp_checkcommand;
my $temp_perfdata;
...
# Now insert junk
for($i; $i < $opt_loops; $i++) {
    $temp_hostname = _randomnessHost();
    $temp_servicename = _randomnessService();
    $temp_checkcommand = _randomcheckCommand();
    $temp_perfdata = _randomperfData();
    $load = "$temp_hostname $temp_servicename $temp_checkcommand \| $temp_perfdata";
    my $handle = $tasks->add_task($opt_queue => $load);
    _out("inserted $i out of $opt_loops, data: $load") if $opt_verbose >= 1; 
}
...
sub _randomnessHost {
    ...
    if (scalar @opt_hosts > 0){
        ...
        $random_hostname = $foo->randpattern("CCCCCnnc");
    }
...
sub _randomnessService {
    ...
    if (scalar @opt_services > 0){
        ...
        $random_servicename = $foo->randpattern("CCCcc");
    }
...
#################################################
sub _randomcheckCommand {
    my $random_checkcommand;
    $random_checkcommand = $foo->randpattern("cccccccccccccccccccccccccccCCCC");
...
sub _randomperfData {
    my $random_perfData;
    my $min = $foo->randpattern("nnnn");
    my $max = $foo->randpattern("nnnn");
    my $warn = $foo->randpattern("nnnn");
    my $crit = $foo->randpattern("nnnn");
    my $value = $foo->randpattern("nnnn");
    $random_perfData = "data=$value;$max;$min;$warn;$crit";
