...
of distributing active Nagios checks across your network to increase
...
exceuting checks. It consists of two parts: There is a NEB module
which resides in the Nagios core and adds servicechecks, hostchecks
and eventhandler to a http://gearman.org[Gearman] queue. The
counterpart is one or more worker clients for the checks itself.
Worker can be configured to only run checks for specific host- or
servicegroups.


* Latest
http://labs.consol.de/wp-content/uploads/2010/09/mod_gearman-1.0.1.tar.gz[version]
is the 1.0.1 which has been released on March 03 2011.
version
...
* Mod-Gearman has been succesfully tested with Nagios 3.2.3 and Icinga 1.2.0 and there are no known bugs at the moment.
  Let me know if you fine one.
* Please use the following https://groups.google.com/group/mod_gearman[google groups mailinglist] for support, questions and bug reports.
...
Mod-Gearman. You just have to enable distributed monitoring by
mod-gearman.
...
* http://labs.consol.de/wp-content/uploads/2010/09/mod_gearman-1.0.1.tar.gz[version 1.0.1 - March 03 2011]
